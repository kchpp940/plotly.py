...
docs = dict(
    ...
    field_display=[
        "dict (default `{}`)",
        "Unified configuration for field display names across axes, legends, hover tooltips, and trace names.",
        "This parameter provides a single source of truth for how data fields are displayed,",
        "working consistently across wide-form, long-form, facet, and animation scenarios.",
        "",
        ...
        "    )",
        "",
        "The three sub-dicts are completely independent name-spaces that never collide:",
        '  - `by_role`: keyed by semantic role — "x", "y", "color", "size", "symbol",',
        '    "line_dash", "pattern_shape", "facet_row", "facet_col", "animation_frame",',
        '    "line_group", "hover_name", "dimensions", "trendline", etc.',
        ...
        '    wide-form unpivot ("value", "variable", "index") or renamed-duplicate columns.',
        "",
        "**Flat dict** (backward-compatible)::",
        "",
        '    field_display={"x": "横轴", "sales": "销售额", "value": "数值"}',
        "",
        "Keys that match a known semantic role are auto-classified into `by_role`,",
        "keys that match a known internal derived name are auto-classified into `by_internal`,",
        "and everything else goes to `by_column`.",
        "",
        "Values can be:",
        "  - A string: the display name for the field",
        "  - `False`: hide the field from hover tooltips",
        "",
        "When both `labels` and `field_display` are provided, `field_display` takes precedence.",
        "The `labels` parameter is absorbed into the context at construction time so that",
        "downstream code never reads `labels` directly.",
    ],
    category_orders=[
